...
def get_all_symbols_connections(labeled_crops: List[Tuple[str, Box]]) -> List[List[Tuple[int, float]]]:
    ...
    for b1_i, b1 in enumerate(labeled_crops):
        for b2_i, b2 in enumerate(labeled_crops):
            ...
            relations[b1_i].append((b2_i, get_most_probable_relation(b1, b2)))
...
def get_minimum_spanning_tree_symbol_connections(labeled_crops: List[Tuple[str, Box]]) -> List[Tuple[int, int]]:
